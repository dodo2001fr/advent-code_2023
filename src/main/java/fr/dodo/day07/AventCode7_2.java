...
public class AventCode7_2 {

    // 0 diff, 1 one pair, 2 two pair, 3 three, 4 full house (2+3), 5 - four card, 6
    // - five card
    static int FIVE = 6;
    static int CARRE = 5;
    static int FULL = 4;
    static int THREE = 3;
    static int DOUBLE_PAIR = 2;
    static int PAIR = 1;
    static int DIFF = 0;
    ...
    public static void main(String[] args) throws IOException {
        ...
        AventCode7_2 instance = new AventCode7_2();
    ...
    public void process(List<String> lines) {
        ...
        Collections.sort(handList, new Comparator<Hand>() {
            ...
            public int compare(Hand left, Hand right) {
                ...
                for (int i = 0; i < left.card.length(); i++) {
                    ...
                    if (leftChar == 'J' && rightChar != 'J') return -1;
                    if (leftChar != 'J' && rightChar == 'J') return 1;
                    ...
                    if (isNumeric(leftChar) && isNumeric(rightChar)) {
                        ...
                        if (leftChar == 'A') {
                            ...
                            if (rightChar == 'Q')
                                continue;
                            else if (rightChar == 'A' || rightChar == 'K')
                                return -1;
                            else
                                return 1;
                        } else if (leftChar == 'J') {
                            if (rightChar == 'J')
                                continue;
                            else if (rightChar == 'A' || rightChar == 'K' || rightChar == 'Q')
                                return -1;
                            else
                                return 1;
                        } else if (leftChar == 'T') {
        ...
        int rank = 1;
        String previous = "";
        for (int i = 1; i < handList.size() + 1; i++) {
        Hand hand = handList.get(i - 1);
        if (!previous.equals(hand.card)) {
        rank = i;
        }
